...
def predict():
    ...
    cm = metrics.ConfusionMatrix(num_classes=2, output_transform=binary_one_hot_output_transform, device=device)
    precision = metrics.Precision()
    recall = metrics.Recall()
    dice_coefficient = metrics.DiceCoefficient(cm, ignore_index=0)
    miou = metrics.mIoU(cm, ignore_index=0)
